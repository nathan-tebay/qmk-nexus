...
export function KeycodePicker({ onSelect, onClose, currentCode = '', layerCount = 5, inline = false }: Props) {
  ...
  const [macroMode, setMacroMode] = useState(() => /^M\(\s*\d+\s*\)$/.test(currentCode))
    ...
    <>
      <div className={styles.holdTapRow}>
        <button
          ...
          onClick={() => { setHoldTap((v) => !v); setMacroMode(false); setTapCode('') }}
          title="Create a key that sends one code when tapped and another action when held"
        >
          Hold / Tap
        </button>
        <button
          className={`${styles.toggleBtn} ${macroMode ? styles.toggleActive : ''}`}
          onClick={() => { setMacroMode((v) => !v); setHoldTap(false); setTapCode('') }}
          title="Build a multi-step macro and assign it as M(n)"
        >
          Macro
        </button>
        <button
      ...
      {macroMode ? (
        <MacroPanel currentCode={currentCode} onSelect={onSelect} onClose={onClose} />
      ) : holdTap ? (
        ...
        <>
          ...
          <div className={styles.modRow}>
            <span className={styles.modLabel}>Modifiers:</span>
            {MODS.map((m) => (
              <button
                key={m.key}
