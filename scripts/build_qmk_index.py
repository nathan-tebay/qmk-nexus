...
def build(kb_root: Path) -> list[dict]:
    ...
    for dir_path, file_path in sorted(candidates.items()):
        ...
        if data is None:
            skipped += 1
            continue
        if _is_native_priority(rel):
            _mark_native_qmk(data, qmk_root, rel)
        elif _unsupported_matrix(data):
            unsupported += 1
            continue
    ...
    print(f'  unsupported {unsupported} (custom/direct matrix not generated by Nexus)')
